...
import com.ota.common.model.highchart.Chart;
import com.ota.ext.Model;
import com.ota.ext.ShiroExt;
import com.ota.util.IpUtil;
...
public class Log extends Model<Log> {
	...
	public Chart chart(Form f) {
		boolean flag = false;
		if (f.getFromParm("operation") == null)
			flag = true;

		if (flag)
			f.setFromParm("operation", "1");
		List<Log> date = dao.find(sql("system.log.chart") + f.getWhereGroupDate());
		if (flag)
			f.setFromParm("operation", "2");
		...
		if (!flag && f.getFromParm("operation").equals("1"))
			chart.series.remove(1);
		else if (!flag)
