...
public class Fn {
	/**
	 * 时间间隔枚举. 此枚举供{@link Fn#dateadd(Fn.interval, int, java.util.Date)}和
	 ...
	 * 
	 * @param interval
	 *            要添加的间隔的类型，支持以下几种类型：<br />
	 *            　　Y（年） M（月） D（日） h（时） m（分） s（秒） w(周)
	 * @param number
	 *            要添加的间隔数.
	 *            如果为正数，将返回参数datetime之后的日期时间；如果为负，将返回datetime之前的日期时间；如果为零
	 *            ，返回datetime本身.
	 * @param datetime
	 *            指定日期时间.
	...
	/**
	 * 返回两个日期时间之间的间隔. 实现类似MSSQL数据库datediff函数的功能. <br />
	 * 如果date1早于date2，将返回正数；如果date1晚于date2, 将返回负数；如果相等，将返回0.
	 * 
	 * @param interval
	 *            为返回的间隔的类型. 有以下几种类型：<br />
	 *            　　Y（年） M（月） D（日） h（时） m（分） s（秒） <br/>
