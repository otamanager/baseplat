...
/**
 * JDiy图片操作工具类. 此类封装简化了图片大小获取/缩放调整，水印等常用功能.
 */
...
public final class Pic {
	...
	public Pic(File file) {
		...
		// if (this.file.isDirectory()) throw new
		// IOException(" file is directory");

		String fn = file.getName().toLowerCase();
		opacityType = fn.endsWith(".png") || fn.endsWith("gif") ? BufferedImage.TYPE_INT_ARGB
	...
	/**
	 * 将图片等比缩放至w,h限定的范围之内,此范围之内空的区域以指定的颜色进行填充. 与{@link #resizeBy(int, int)}
	 * 不同的是，该方法缩小后的图片(为了自尺寸与原图等比)长宽并不一定会等于w,h的值, 而此方法缩放后的图片，其长度一定等于w,
	 * 宽度一定等于h,但是图片的显示画面仍然与原图保持等比，中间空出来的地方，将会用指定的颜色填充.
	 * 
	 * @param w
	 *            缩小后的图片宽度
	 * @param h
	...
	/**
	 * 将图片收缩或放大至指定尺寸. 此方法不保证等比，如果图片原尺寸的比例和传入的参数w/h不成比例，图片将被拉伸变形.
	 * 
	 * @param w
	 *            缩放后的图片宽度
	 * @param h
